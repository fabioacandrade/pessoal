...
class Q07{
    ...
    public static void main(String[] args){
        ...
        //ordenando por inserção utilizando o criterio anonascimento e em caso de empate o nome
        for(int i=1;i<countCopia;i++){
            Jogador tmp = copia[i];
            int j = i-1;
            comparacoes++;
            while((j>=0) && (copia[j].getAnoNascimento() > tmp.getAnoNascimento() || (copia[j].getAnoNascimento() == tmp.getAnoNascimento() && copia[j].getNome().compareTo(tmp.getNome()) > 0))){
                copia[j+1] = copia[j];
                j--;
                countMove++;
                comparacoes++;
        ...
        matricula.openWrite("matricula_insercao.txt");
