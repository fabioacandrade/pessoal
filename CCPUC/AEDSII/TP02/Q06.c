...
void selecaoRecursiva(Jogador *jogador,int esq,int dir,int *countComparacoes,int *countTrocas){
    int i=esq,j=dir;
    Jogador pivo = jogador[(esq+dir)/2];
    Jogador aux;
    while(i<=j){
        while(strcmp(jogador[i].nome,pivo.nome)<0){
            i++;
            (*countComparacoes)++;
        }
        while(strcmp(jogador[j].nome,pivo.nome)>0){
            j--;
            (*countComparacoes)++;
        }
        if(i<=j){
    ...
    if(esq<j){
        selecaoRecursiva(jogador,esq,j,countComparacoes,countTrocas);
    }
    if(i<dir){
        selecaoRecursiva(jogador,i,dir,countComparacoes,countTrocas);
    }
}
...
int main(){
    ...
    //ordenando o array copia Recursivamente por selecao
    selecaoRecursiva(copia,0,countCopia-1,&countComparacoes,&countTrocas);
